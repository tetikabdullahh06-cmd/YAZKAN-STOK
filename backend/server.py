...
async def toolholder_scrap_pdf(scrap_id: str, user=Depends(get_current_user)):
    ...
    pdf.drawString(250, height - 55, "YAZKAN DÖKÜM SANAYİ VE TİCARET A.Ş.")
    pdf.drawString(250, height - 68, "A: ASO 2. O.S.B. 2011 Cad. No:21 Temelli")
    pdf.drawString(250, height - 81, "ANKARA - TURKEY  |  T: +90 (312) 641 32 10")
    doc_x, doc_y, doc_w, doc_h = width - 190, height - 96, 135, 58
    pdf.setStrokeColorRGB(0.2, 0.25, 0.32); pdf.setLineWidth(0.6); pdf.rect(doc_x, doc_y, doc_w, doc_h, stroke=1, fill=0)
    pdf.line(doc_x, doc_y + 39, doc_x + doc_w, doc_y + 39); pdf.line(doc_x, doc_y + 20, doc_x + doc_w, doc_y + 20)
    pdf.setFont(bold_font, 6.8); pdf.setFillColorRGB(0.12, 0.16, 0.22)
    pdf.drawString(doc_x + 5, doc_y + 45, "Doküman No:")
    pdf.drawString(doc_x + 5, doc_y + 26, "Doküman Rev. No:")
    pdf.drawString(doc_x + 5, doc_y + 7, "Değ. Nedeni:")
    pdf.setFont(regular_font, 7.5)
    pdf.drawRightString(doc_x + doc_w - 5, doc_y + 45, scrap.get("document_no") or "T4.000")
    pdf.drawRightString(doc_x + doc_w - 5, doc_y + 26, scrap.get("document_revision") or "00")
    pdf.drawRightString(doc_x + doc_w - 5, doc_y + 7, scrap.get("change_reason") or "-")
