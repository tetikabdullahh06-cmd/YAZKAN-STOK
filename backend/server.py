...
async def toolholder_scrap_pdf(scrap_id: str, user=Depends(get_current_user)):
    ...
    pdf.drawString(250, height - 50, "YAZKAN DÖKÜM SANAYİ VE TİCARET A.Ş.")
    pdf.drawString(250, height - 63, "A: ASO 2. O.S.B. 2011 Cad. No:21 Temelli")
    pdf.drawString(250, height - 76, "ANKARA - TURKEY | T: +90 (312) 641 32 10")
    doc_x, doc_y, doc_w, doc_h = width - 170, height - 92, 115, 54
    pdf.setFillColorRGB(1, 1, 1); pdf.setStrokeColorRGB(0.2, 0.25, 0.32); pdf.setLineWidth(0.6); pdf.rect(doc_x, doc_y, doc_w, doc_h, stroke=1, fill=1)
    pdf.line(doc_x, doc_y + 36, doc_x + doc_w, doc_y + 36); pdf.line(doc_x, doc_y + 18, doc_x + doc_w, doc_y + 18)
    pdf.setFont(bold_font, 6.1); pdf.setFillColorRGB(0.12, 0.16, 0.22)
    pdf.drawString(doc_x + 4, doc_y + 42, "Doküman No:")
    pdf.drawString(doc_x + 4, doc_y + 24, "Doküman Rev. No:")
    pdf.drawString(doc_x + 4, doc_y + 6, "Değ. Nedeni:")
    pdf.setFont(regular_font, 6.8)
    pdf.drawRightString(doc_x + doc_w - 4, doc_y + 42, scrap.get("document_no") or "T4.000")
    pdf.drawRightString(doc_x + doc_w - 4, doc_y + 24, scrap.get("document_revision") or "00")
    pdf.drawRightString(doc_x + doc_w - 4, doc_y + 6, scrap.get("change_reason") or "-")
