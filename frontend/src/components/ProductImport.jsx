...
/**
 * Excel bulk import modal for products.
 * Props: onClose(), onCommitted() — refreshes parent list
 */
export default function ProductImport({ onClose, onCommitted }) {
  ...
  const runPreview = async (f) => {
    ...
    try {
      ...
      const r = await fetch(`${API}/products/import?commit=false`, {
        method: "POST",
        credentials: "include",
  ...
  const commit = async () => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const r = await fetch(`${API}/products/import?commit=true`, {
        method: "POST",
        credentials: "include",
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        body: fd,
  ...
  const downloadTemplate = async () => {
    try {
      ...
      a.href = url; a.download = "urun_sablonu.xlsx";
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700">
          <div>
            ...
            <div className="font-display text-xl font-bold">Ürünleri Excel'den İçe Aktar</div>
          </div>
          <button onClick={onClose} data-testid="import-close" className="p-2 rounded-lg hover:bg-slate-800 text-slate-400"><X className="w-5 h-5" /></button>
        </div>

        <div className="p-6 flex-1 overflow-auto space-y-4">
          <div className="flex flex-wrap gap-3">
            <button onClick={downloadTemplate} data-testid="import-template"
            ...
            <input ref={inputRef} type="file" accept=".xlsx,.xls" onChange={onFile} className="hidden" data-testid="import-file" />
            <button onClick={() => inputRef.current?.click()} data-testid="import-pick"
          ...
          <div className="text-xs text-slate-500 bg-slate-800/40 border border-slate-700 rounded-lg p-3">
            <strong>Format:</strong> Başlık satırı zorunlu — <span className="font-mono text-slate-300">code, name, category, unit, unit_price, min_stock, current_stock</span>. code/name/category boş bırakılamaz.
            Kayıtlı bir kod bulunursa <span className="text-amber-300 font-semibold">güncellenir</span>, yoksa <span className="text-emerald-300 font-semibold">oluşturulur</span>.
          </div>
            ...
            <div className="bg-slate-800/40 border border-slate-700 rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-slate-900/60 text-slate-400 text-xs uppercase tracking-wider">
                  <tr><th className="px-3 py-2 text-left">Satır</th><th className="px-3 py-2 text-left">İşlem</th><th className="px-3 py-2 text-left">Kod</th><th className="px-3 py-2 text-left">Ad</th><th className="px-3 py-2 text-left">Kategori</th><th className="px-3 py-2 text-right">Stok</th><th className="px-3 py-2 text-right">Fiyat</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-700">
                  {preview.preview.map((p) => (
                    <tr key={p.row} className={p.action === "skip" ? "bg-red-950/20" : ""}>
                      ...
                      <td className="px-3 py-2 font-mono-tab">{p.data.code}</td>
                      <td className="px-3 py-2">{p.data.name || <span className="text-red-800 italic text-xs">{p.error}</span>}</td>
                      <td className="px-3 py-2 text-slate-400">{p.data.category}</td>
                      <td className="px-3 py-2 text-right font-mono-tab">{p.data.current_stock ?? "-"}</td>
                      <td className="px-3 py-2 text-right font-mono-tab">{p.data.unit_price != null ? `₺${p.data.unit_price}` : "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="px-6 py-4 border-t border-slate-700 flex justify-end gap-3">
          ...
          <button onClick={commit} disabled={!preview || committing || (stats && stats.create + stats.update === 0)} data-testid="import-commit"
