...
export default function Orders() {
  ...
  const productQuality = (p) => p.quality || p.kalite || p.grade || p.quality_grade || "Kalite belirtilmemiş";
  const productStock = (p) => p.current_stock ?? p.stock ?? p.quantity ?? 0;
  const toolholderQuality = (h) => h.quality || h.kalite || h.grade || h.quality_grade || "Kalite belirtilmemiş";
  const toolholderStock = (h) => h.current_stock ?? h.stock ?? h.quantity ?? 0;
