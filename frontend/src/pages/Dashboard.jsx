...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div data-testid="dashboard-out-of-stock" className="bg-rose-950/30 border-2 border-rose-700/60 rounded-2xl overflow-hidden shadow-lg shadow-rose-950/20"><div className="px-6 py-4 border-b border-rose-700/50 flex items-center justify-between gap-3 bg-rose-500/10"><div><h3 className="font-display text-xl font-black text-rose-200">Stoğu Biten Ürünler</h3><p className="text-sm text-rose-300/80">Mevcut stok miktarı sıfır olan ürünler</p></div><span className="rounded-full bg-rose-600 px-3 py-1 text-sm font-black text-white">{d.out_of_stock_products?.length || 0} kayıt</span></div>{(d.out_of_stock_products || []).length > 0 ? <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-rose-900/40 text-rose-100"><tr><th className="px-6 py-3 text-left">Ürün</th><th className="px-6 py-3 text-left">Marka</th><th className="px-6 py-3 text-left">Kategori</th><th className="px-6 py-3 text-left">Mevcut Stok</th><th className="px-6 py-3 text-left">Minimum Stok</th><th className="px-6 py-3 text-left">Konum</th></tr></thead><tbody className="divide-y divide-rose-900/50">{d.out_of_stock_products.map((p) => <tr key={p.id} className="hover:bg-rose-500/5"><td className="px-6 py-3 font-bold text-white">{p.name}<div className="text-xs text-rose-300/70 font-mono-tab">{p.code}</div></td><td className="px-6 py-3 text-slate-200">{p.brand || "-"}</td><td className="px-6 py-3 text-slate-200">{p.category || "-"}</td><td className="px-6 py-3 font-black text-rose-300">0 {p.unit || "adet"}</td><td className="px-6 py-3 text-slate-200">{p.min_stock || 0} {p.unit || "adet"}</td><td className="px-6 py-3 text-slate-200">{p.location || "-"}</td></tr>)}</tbody></table></div> : <div className="p-6 text-sm font-bold text-emerald-300">Stoğu tamamen biten ürün bulunmuyor.</div>}</div>
