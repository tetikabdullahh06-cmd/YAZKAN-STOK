...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div data-testid="dashboard-out-of-stock" className="relative bg-rose-50 border-2 border-rose-300 rounded-2xl overflow-hidden shadow-lg shadow-rose-900/30"><div className="absolute inset-0 pointer-events-none opacity-20" style={{ backgroundImage: "repeating-linear-gradient(45deg, transparent 0 12px, rgba(255,255,255,0.12) 12px 24px)" }} /><div className="relative px-6 py-4 border-b border-rose-200 flex items-center gap-3 bg-rose-100"><div className="w-11 h-11 rounded-lg bg-white border border-rose-300 flex items-center justify-center"><Package className="w-6 h-6 text-rose-700" /></div><div className="flex-1"><div className="text-[10px] text-rose-900 uppercase tracking-[0.3em] font-black">Acil Dikkat</div><h3 className="font-display text-xl font-black text-slate-950">Stoğu Biten Ürünlerin Tam Listesi · {d.out_of_stock_products?.length || 0} kalem</h3></div><Link to="/urunler" className="hidden md:inline-flex h-10 px-4 rounded-lg bg-red-600 hover:bg-red-500 text-white text-sm font-bold items-center gap-2">Ürünleri Gör</Link></div>{(d.out_of_stock_products || []).length > 0 ? <div className="relative divide-y divide-rose-200 max-h-[32rem] overflow-y-scroll overflow-x-hidden bg-white" style={{ scrollbarWidth: "auto", scrollbarGutter: "stable" }}>{d.out_of_stock_products.map((p) => <div key={p.id} className="flex items-center justify-between px-6 py-3 hover:bg-rose-50"><div className="min-w-0"><div className="font-mono-tab text-xs font-bold text-rose-800">{p.code}</div><div className="font-semibold text-slate-950 truncate">{p.name}</div>{p.location && <div className="text-xs text-slate-700 font-semibold">Konum: {p.location}</div>}</div><div className="text-right shrink-0 ml-4"><div className="text-2xl font-black font-mono-tab text-slate-950">0 <span className="text-sm text-slate-700">/ {p.min_stock}</span></div><div className="text-[10px] text-slate-700 uppercase tracking-wider font-bold">mevcut / min · {p.unit}</div></div></div>)}</div> : <div className="relative px-6 py-5 text-sm font-bold text-emerald-300">Stoğu tamamen biten ürün bulunmuyor.</div>}</div>
