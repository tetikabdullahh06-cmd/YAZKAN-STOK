...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div data-testid="dashboard-out-of-stock" className="relative bg-gradient-to-br from-red-950/60 via-red-900/40 to-red-950/60 border-2 border-red-500/50 rounded-2xl overflow-hidden shadow-lg shadow-red-950/40"><div className="absolute inset-0 pointer-events-none opacity-30" style={{ backgroundImage: "repeating-linear-gradient(45deg, transparent 0 12px, rgba(239,68,68,0.08) 12px 24px)" }} /><div className="relative px-6 py-4 border-b border-red-500/30 flex items-center gap-3 bg-red-500/10"><div className="w-11 h-11 rounded-lg bg-red-500/20 border border-red-500/40 flex items-center justify-center"><Package className="w-6 h-6 text-red-300" /></div><div className="flex-1"><div className="text-[10px] text-red-300 uppercase tracking-[0.3em] font-bold">Acil Dikkat</div><h3 className="font-display text-xl font-black text-red-100">Stoğu Biten Ürünler · {d.out_of_stock_products?.length || 0} kalem</h3></div><Link to="/urunler" className="hidden md:inline-flex h-10 px-4 rounded-lg bg-red-600 hover:bg-red-500 text-white text-sm font-bold items-center gap-2">Ürünleri Gör</Link></div>{(d.out_of_stock_products || []).length > 0 ? <div className="relative divide-y divide-red-900/40 max-h-96 overflow-auto">{d.out_of_stock_products.map((p) => <div key={p.id} className="flex items-center justify-between px-6 py-3 hover:bg-red-500/5"><div className="min-w-0"><div className="font-mono-tab text-xs text-red-300">{p.code}</div><div className="font-semibold text-red-50 truncate">{p.name}</div>{p.location && <div className="text-xs text-red-300/70">Konum: {p.location}</div>}</div><div className="text-right shrink-0 ml-4"><div className="text-2xl font-black font-mono-tab text-red-300">0 <span className="text-sm text-red-400/60">/ {p.min_stock}</span></div><div className="text-[10px] text-red-300/70 uppercase tracking-wider">mevcut / min · {p.unit}</div></div></div>)}</div> : <div className="relative px-6 py-5 text-sm font-bold text-emerald-300">Stoğu tamamen biten ürün bulunmuyor.</div>}</div>
