...
export default function Gloves() {
  ...
  return <div className="space-y-6">
    ...
    {!showHistory ? <>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4"><div className="rounded-2xl border border-cyan-700/60 bg-gradient-to-br from-cyan-950/80 to-slate-900 p-5"><div className="text-sm font-bold text-cyan-200">Eldiven Çeşidi</div><div className="mt-1 text-3xl font-black text-white">{filteredGloves.length}</div><div className="text-xs text-slate-400">listelenen stok kartı</div></div><div className="rounded-2xl border border-emerald-700/60 bg-gradient-to-br from-emerald-950/80 to-slate-900 p-5"><div className="text-sm font-bold text-emerald-200">Toplam Eldiven Stoku</div><div className="mt-1 text-3xl font-black text-emerald-300">{filteredGloves.reduce((sum, g) => sum + Number(g.current_stock || 0), 0)}</div><div className="text-xs text-slate-400">stok kartlarının toplamı</div></div><div className="rounded-2xl border border-rose-700/60 bg-gradient-to-br from-rose-950/80 to-slate-900 p-5"><div className="text-sm font-bold text-rose-200">Kritik Stok</div><div className="mt-1 text-3xl font-black text-rose-300">{filteredGloves.filter((g) => Number(g.current_stock || 0) <= Number(g.min_stock || 0)).length}</div><div className="text-xs text-slate-400">minimum seviyede veya altında</div></div></div>
      <div className="rounded-2xl border border-rose-600/70 bg-rose-950/30 overflow-hidden"><div className="px-5 py-4 border-b border-rose-800/60 flex items-center justify-between gap-3"><div><h2 className="text-xl font-black text-rose-200">Stoğu Biten Eldivenler</h2><p className="text-sm text-rose-300/80">Mevcut stok miktarı sıfır olan eldivenler</p></div><span className="rounded-full bg-rose-600 px-3 py-1 text-sm font-black text-white">{outOfStockGloves.length} kayıt</span></div>{outOfStockGloves.length > 0 ? <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-rose-900/40 text-rose-100"><tr><th className="p-3 text-left">Eldiven</th><th className="p-3 text-left">Marka</th><th className="p-3 text-left">Beden</th><th className="p-3 text-left">Mevcut Stok</th><th className="p-3 text-left">Minimum Stok</th><th className="p-3 text-left">Konum</th></tr></thead><tbody>{outOfStockGloves.map((g) => <tr key={g.id} className="border-t border-rose-900/50"><td className="p-3 font-black text-white">{g.name}</td><td className="p-3 text-slate-200">{g.brand || "-"}</td><td className="p-3 text-slate-200">{g.size || "-"}</td><td className="p-3 font-black text-rose-300">0 {g.unit || "çift"}</td><td className="p-3 text-slate-200">{g.min_stock || 0} {g.unit || "çift"}</td><td className="p-3 text-slate-200">{g.location || "-"}</td></tr>)}</tbody></table></div> : <div className="p-5 text-sm font-bold text-emerald-300">Stoğu tamamen biten eldiven bulunmuyor.</div>}</div>
      <div className="rounded-2xl border border-cyan-700/70 bg-gradient-to-br from-cyan-950/80 to-slate-900 overflow-hidden"><div className="px-5 py-4 border-b border-cyan-700/60 flex items-center justify-between gap-3"><div><h2 className="text-xl font-black text-cyan-100">Personel Eldiven Kullanımı</h2><p className="text-sm text-cyan-200/80">Kime kaç adet verildiği ve hangi eldiveni kullandığı</p></div><span className="rounded-full bg-cyan-600 px-3 py-1 text-sm font-black text-white">{allPersonSummary.length} personel</span></div>{allPersonSummary.length > 0 ? <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-cyan-900/50 text-cyan-50"><tr><th className="p-3 text-left">Personel</th><th className="p-3 text-left">Aldığı eldivenler</th><th className="p-3 text-right">Toplam adet</th></tr></thead><tbody>{allPersonSummary.map((person) => <tr key={person.name} className="border-t border-cyan-800/60 hover:bg-cyan-500/10"><td className="p-3 font-black text-white">{person.name}</td><td className="p-3 text-slate-200">{Object.values(person.gloves).map((g) => `${g.name}${g.size ? ` • ${g.size}` : ""}${g.brand ? ` • ${g.brand}` : ""}: ${g.quantity}`).join("  |  ")}</td><td className="p-3 text-right font-black text-cyan-200">{person.total}</td></tr>)}</tbody></table></div> : <div className="p-5 text-sm font-bold text-slate-300">Henüz personele verilmiş eldiven kaydı bulunmuyor.</div>}</div>
