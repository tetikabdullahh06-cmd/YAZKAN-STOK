...
export default function Gloves() {
  ...
  return <div className="space-y-6">
    ...
    {!showHistory ? <>
      ...
      {outOfStockGloves.length > 0 && <div className="rounded-2xl border border-rose-600/70 bg-rose-950/30 overflow-hidden"><div className="px-5 py-4 border-b border-rose-800/60 flex items-center justify-between gap-3"><div><h2 className="text-xl font-black text-rose-200">Stoğu Biten Eldivenler</h2><p className="text-sm text-rose-300/80">Mevcut stok miktarı sıfır olan eldivenler</p></div><span className="rounded-full bg-rose-600 px-3 py-1 text-sm font-black text-white">{outOfStockGloves.length} kayıt</span></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-rose-900/40 text-rose-100"><tr><th className="p-3 text-left">Eldiven</th><th className="p-3 text-left">Marka</th><th className="p-3 text-left">Beden</th><th className="p-3 text-left">Mevcut Stok</th><th className="p-3 text-left">Minimum Stok</th><th className="p-3 text-left">Konum</th></tr></thead><tbody>{outOfStockGloves.map((g) => <tr key={g.id} className="border-t border-rose-900/50"><td className="p-3 font-black text-white">{g.name}</td><td className="p-3 text-slate-200">{g.brand || "-"}</td><td className="p-3 text-slate-200">{g.size || "-"}</td><td className="p-3 font-black text-rose-300">0 {g.unit || "çift"}</td><td className="p-3 text-slate-200">{g.min_stock || 0} {g.unit || "çift"}</td><td className="p-3 text-slate-200">{g.location || "-"}</td></tr>)}</tbody></table></div></div>}
