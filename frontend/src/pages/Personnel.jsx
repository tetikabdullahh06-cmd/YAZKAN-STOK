...
export default function Personnel() {
  ...
  const exportExcel = async () => { const rows = filtered.map((p) => ({ "Ad": p.first_name, "Soyad": p.last_name, "Görev": p.department || "", "Görsel": "", __imageUrl: p.image_url || "" })); await downloadImageWorkbook({ sheetName: "Personel", rows, imageKey: "Görsel", imageSourceKey: "__imageUrl", filename: `YAZKAN-personel-${new Date().toISOString().slice(0, 10)}.xlsx` }); toast.success(`${rows.length} personel dışa aktarıldı. ${imageExportNotice(rows)}`); };
